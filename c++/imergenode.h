...
/// Input stream merging two or more SAM/BAM input streams. These nodes
/// are connected as a tree. Calling operator>> on the root node recursively
/// applies >> to all the descendents.
class imergenode {
public:
  /// Construct a merged stream from two opened input streams
  imergenode(isamstream* s1) : stream1(s1), stream2(NULL) { }
  ~imergenode() {
    if (stream1 != NULL) delete stream1;
    if (stream2 != NULL) delete stream2;
  }
  imergenode* append(isamstream* s) {
      imergenode* tmp = new imergenode(s);
      this->stream2 = tmp;
      return tmp;
  }
  ...
  bool operator>> (collection& headers) {
    ...
    if (stream2 != NULL) {
        ...
        if ((*stream1) >> a1) {
            state = ((*stream2) >> a2)? bothpending : pending1;
        }
        else  state = only2;
    }
    return true;
  ...
  bool operator>> (alignment& aln) {
    ...
    case pending1:
      aln.swap(a1); state = only1; return true;

    case pending2:
      aln.swap(a2); state = only2; return true;
